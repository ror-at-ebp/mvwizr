# mvwizr ![](reference/figures/mvwizr_logo.png)
...
- `?funktion`: Zeigt die Hilfe für eine bestimmte Funktion an, also z.B.
  [`?einlesen_mv_gbl`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_mv_gbl.md).
...
- [`einlesen_nawa()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_nawa.md):
  Liest einzelne MV-Dateien (oder Dataframes) im NAWA-Format ein,
  normalisiert Einheiten, entfernt Duplikate, bestimmt
...
- [`batch_einlesen_nawa()`](https://ror-at-ebp.github.io/mvwizr/reference/batch_einlesen_nawa.md):
  ...
  Import-Manifest oder eine Liste von Dateipfaden. Die Funktion kann
  auch die Parameter wie Encoding, Header, Delimiter und Sprache
  erraten, wenn diese nicht angegeben sind in Form eines
...
- `schreibe_schreibe_nawa_import_manifest_template()`: Erstellt ein
  ...
  [`batch_einlesen_nawa()`](https://ror-at-ebp.github.io/mvwizr/reference/batch_einlesen_nawa.md)
  verwendet werden kann. Der Pfad zu den MV-Dateien kann angegeben
  werden um diese in das Template zu schreiben. Das Manifest kann dann
  angepasst werden, um die Parameter für das Einlesen der NAWA-Daten zu
  definieren.
- [`einlesen_mv_gbl()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_mv_gbl.md):
  Liest die MV-Daten des GBL ein, normalisiert Einheiten, entfernt
  Duplikate, bestimmt Bestimmungsgrenzen aus Daten und verknüpft Daten
  mit der VSA Substanz_ID. Test-Daten werden mitgeliefert (siehe
  Beispiele im Hilfetext der Funktion).
- [`einlesen_bafu_lookup()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_bafu_lookup.md):
  Liest die Lookup-Tabelle
...
- [`einlesen_kriterien()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_kriterien.md):
  Liest die Qualitätskriterien des VSA ein und entfernt Duplikate
  (Bevorzugung tieferer CQK und AQK für die Auswahl). Eine Kopie der
  Datei wird mitgeliefert (siehe Beispiele im Hilfetext der Funktion).
- [`einlesen_regulierungen()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_regulierungen.md):
  Liest die MV-Regulierungen des VSA ein (diese Tabelle ist immer nur
  ein Auszug aus der VSA-Datenbank). Eine aktuelle Kopie der Datei wird
  mitgeliefert (siehe Beispiele im Hilfetext der Funktion).
- [`einlesen_vsa_lookup()`](https://ror-at-ebp.github.io/mvwizr/reference/einlesen_vsa_lookup.md):
  Liest die VSA-Lookup-Tabelle ein, die ein Bezug zwischen Parameter-ID
  des BAFU und der VSA Substanz_ID herstellt. Eine aktuelle Kopie der
  Datei wird mitgeliefert (siehe Beispiele im Hilfetext der Funktion).
...
(siehe
[`vignette("datei-spezifikationen")`](https://ror-at-ebp.github.io/mvwizr/articles/datei-spezifikationen.md)).
Auch werden bereits eingelesene Daten als Objekte im Paket zur Verfügung
gestellt (siehe Vignette).

### Berechnungen

- [`berechne_rq_ue()`](https://ror-at-ebp.github.io/mvwizr/reference/berechne_rq_ue.md):
  Berechnet Risikoquotienten und Überschreitungen für AQK und CQK.
  Output dient als Grundlage für diverse Plot-Funktionen
- [`berechne_mixtox()`](https://ror-at-ebp.github.io/mvwizr/reference/berechne_mixtox.md):
  Berechnet Mischungstoxizitäten aus dem Output von
  [`berechne_rq_ue()`](https://ror-at-ebp.github.io/mvwizr/reference/berechne_rq_ue.md)
...
- [`plot_misch_verlauf()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_verlauf.md):
  Konzentrationsverläufe für Einzelsubstanzen, mehrere Substanzen und
  Summen (auch gruppiert) in verschiedenen Darstellungsarten.
- [`plot_misch_ue()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_ue.md):
  Verlauf von GSchV-Überschreitungen (entweder für kurzzeitige oder
  andauernde Belastungen) pro Substanz und für eine Station.
- [`plot_misch_ue_summe()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_ue_summe.md):
  Verlauf von GSchV-Überschreitungen pro Jahr und Station mit
  Unterscheidung nach Überschreitung von numerischem Grenzwert (GSchV
  Anh. 2) oder allgemeinem Wert für org. Pestizide (0.1 µg/l).
- [`plot_misch_ue_qk()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_ue_qk.md):
  Überschreitungen von QK (für Substanzen ohne spezifischen
  Anforderungswert in GSchV Anh. 2; entweder CQK oder AQK) im Verhältnis
  zu Überschreitungen der Substanzen, die in der GSchV aufgelistet sind.
  Mit Option für detaillierte Darstellung (mit einzelnen Substanzen) für
  den Vergleich verschiedener Stationen.
- [`plot_misch_oekotox_uebersicht()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_oekotox_uebersicht.md):
  Stationsübersicht Ökotoxbeurteilung (Einzelstoffe und
  Mischungstoxizität) pro Station und Jahr. Entweder für kurzzeitige
  oder andauernde Belastungen.
- [`plot_misch_mixtox_verlauf()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_mixtox_verlauf.md):
  Zeitreihe der Mischungstoxizität für mehrere Stationen und Jahre
  gleichzeitig
- [`plot_misch_mixtox_haeufigkeit()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_misch_mixtox_haeufigkeit.md):
  Zeitreihe der Häufigkeitsverteilung der Mischungstoxizität für eine
  Station (für kurzzeitige oder andauernde Belastungen)
- [`plot_stich_uebersicht()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_stich_uebersicht.md):
  Übersicht über Stichproben mittels Raster-Darstellung über die Zeit
  für eine Station. Die Konzentrationen werden dabei durch Farben in
  logarithmischer Skala dargestellt, wodurch schnell relevante
  Substanzen identifiziert werden können.
- [`plot_stich_verlauf()`](https://ror-at-ebp.github.io/mvwizr/reference/plot_stich_verlauf.md):
  Konzentrationsverlauf für Stichproben für mehrere Substanzen und
  Stationen. Gut geeignet für eine interaktive Analyse mit Plotly.
...
![](reference/figures/mvwizr_struktur_funktionen_interfaces.jpg)
...
6.  **Styleguide**: Der Code sollte dem [Tidyverse
    ...
    4.  Funktionsaufrufe sollten immer explizit mit der Angabe des
        ...
        statt [`filter()`](https://rdrr.io/r/stats/filter.html). Dadurch
        entstehen keine Konflikte (z.B. mit
        [`stats::filter()`](https://rdrr.io/r/stats/filter.html)). Die
        Ausnahme davon sind gewisse Operatoren (z.B. Magrittr-Pipe
